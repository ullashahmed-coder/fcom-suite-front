...
export default function MixedPackingDashboard() {
  ...
  const getDynamicStatusBadge = (status: string) => {
    ...
    if (s.includes('CANCEL') || s.includes('RETURN')) {
      if(s === 'RETURN ACCEPTED') return <span className="text-[10px] bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded font-bold uppercase flex items-center gap-1"><CheckSquare size={12}/> Restocked</span>;
      return <span className="text-[10px] bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 px-2 py-0.5 rounded font-bold uppercase flex items-center gap-1"><XCircle size={12} /> Returned</span>;
    }
    return <span className="text-[10px] bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-gray-300 px-2 py-0.5 rounded font-bold uppercase flex items-center gap-1"><Clock size={12} /> {status}</span>;
  }

  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] gap-3">
        <Loader2 className="animate-spin text-emerald-600 dark:text-rose-500" size={40} />
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10 animate-in fade-in duration-300 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800 dark:text-white flex items-center gap-2 uppercase tracking-wider">
            Main Packing
          </h1>
          <p className="text-sm text-slate-500 dark:text-gray-400 font-medium mt-1">Deshio Tati official packaging and dispatch</p>
        </div>
      </div>

      {/* Top Action Buttons (Tabs) */}
      <div className="grid grid-cols-2 gap-4 mb-6">
        
        <div onClick={() => setActiveTab('queue')} className={`p-4 sm:p-5 rounded-2xl border flex flex-col items-center text-center justify-center gap-2 cursor-pointer transition-all ${activeTab === 'queue' ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-600 dark:border-emerald-500 shadow-sm ring-1 ring-emerald-600 dark:ring-emerald-500' : 'bg-white dark:bg-[#1a2421] hover:bg-slate-50 dark:hover:bg-white/5 border-gray-200 dark:border-white/5'}`}>
          <div className={`p-3 rounded-xl ${activeTab === 'queue' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400'}`}><Package size={24} /></div>
          <div>
            <p className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'queue' ? 'text-emerald-800 dark:text-emerald-400' : 'text-slate-500 dark:text-gray-400'}`}>Packing Queue</p>
            <p className={`text-2xl font-black mt-1 ${activeTab === 'queue' ? 'text-slate-800 dark:text-white' : 'text-slate-800 dark:text-gray-200'}`}>{toPackOrders.length}</p>
          </div>
        </div>

        <div onClick={() => setActiveTab('history')} className={`p-4 sm:p-5 rounded-2xl border flex flex-col items-center text-center justify-center gap-2 cursor-pointer transition-all ${activeTab === 'history' ? 'bg-emerald-50 dark:bg-emerald-500/10 border-emerald-600 dark:border-emerald-500 shadow-sm ring-1 ring-emerald-600 dark:ring-emerald-500' : 'bg-white dark:bg-[#1a2421] hover:bg-slate-50 dark:hover:bg-white/5 border-gray-200 dark:border-white/5'}`}>
          <div className={`p-3 rounded-xl ${activeTab === 'history' ? 'bg-emerald-600 text-white' : 'bg-emerald-50 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400'}`}><CheckCircle2 size={24} /></div>
          <div>
            <p className={`text-xs font-bold uppercase tracking-wider ${activeTab === 'history' ? 'text-emerald-800 dark:text-emerald-400' : 'text-slate-500 dark:text-gray-400'}`}>Packed History</p>
            <p className={`text-2xl font-black mt-1 ${activeTab === 'history' ? 'text-slate-800 dark:text-white' : 'text-slate-800 dark:text-gray-200'}`}>{displayedPackedOrders.length}</p>
          </div>
        </div>
      ...
      {activeTab === 'queue' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-300">
          <div className="lg:col-span-7 bg-white dark:bg-[#1a2421] p-5 rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col h-[700px] transition-colors">
            <div className="mb-4 pb-4 border-b border-gray-200 dark:border-white/10">
              ...
              <form onSubmit={handleSearchSubmit} className="mt-3 relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <ScanBarcode size={18} className="text-slate-400 dark:text-gray-500" />
                </div>
                <input
                  type="text"
                  autoFocus
                  placeholder="Scan barcode or type ID/CN Number..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-10 pr-3 py-2.5 bg-slate-50 dark:bg-[#141d1a] border border-slate-200 dark:border-white/10 rounded-lg text-sm font-medium text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:border-emerald-600 focus:ring-1 focus:ring-emerald-600 transition-colors shadow-inner"
                />
              </form>
            </div>
            ...
            <div className="overflow-y-auto pr-2 space-y-3 flex-1 custom-scrollbar">
              {filteredToPackOrders.length === 0 ? (
                <div className="text-center py-20 text-slate-400 dark:text-gray-500 flex flex-col items-center gap-2">
                  <Package size={40} className="opacity-50" />
                  <p>কোনো পেন্ডিং অর্ডার পাওয়া যায়নি!</p>
                </div>
              ) : (
                filteredToPackOrders.map((order) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-all border-2 ${isActive ? 'border-emerald-600 dark:border-emerald-500 bg-emerald-50/50 dark:bg-emerald-500/10 shadow-sm' : 'border-slate-100 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/20 bg-white dark:bg-[#141d1a]'}`}
                    >
                      <div className="relative shrink-0">
                        {firstItem?.product?.imageUrl || firstItem?.product?.thumbnail ? (
                          <img
                            ...
                            alt="product"
                          />
                        ) : <div className="w-14 h-14 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/10"></div>}
                      </div>
                      ...
                      <div className="flex-1 flex justify-between items-center gap-3 overflow-hidden">
                        ...
                        <div className="flex flex-col items-end shrink-0">
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isActive ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-400' : 'bg-slate-100 text-slate-600 dark:bg-white/10 dark:text-gray-400'}`}>
                            {date}
                          </span>
                          <span className="text-[9px] text-slate-400 dark:text-gray-500 font-medium mt-1">
                            {time}
                          </span>
                        </div>

                        <span className="text-[10px] bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 px-2 py-0.5 rounded font-bold uppercase flex items-center gap-1 shrink-0">
                          Pending
                        </span>
                      </div>
                    </div>
          ...
          <div className="lg:col-span-5">
            {!selectedOrder ? (
              <div className="bg-slate-50 dark:bg-[#1a2421] border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl h-[700px] flex flex-col items-center justify-center text-center p-8 transition-colors">
              ...
              <div className="bg-white dark:bg-[#1a2421] rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col h-[700px] overflow-hidden transition-colors">
                ...
                <div className="p-6 flex-1 overflow-y-auto custom-scrollbar">
                  <div className="mb-6">
                    ...
                    <div className="space-y-2 text-sm text-slate-700 dark:text-gray-300 bg-slate-50 dark:bg-[#141d1a] p-4 rounded-xl border border-slate-100 dark:border-white/5">
                      <p className="flex items-center gap-2"><User size={14} className="text-slate-400 dark:text-gray-500" /> {selectedOrder.customer?.name}</p>
                      <p className="flex items-center gap-2"><Phone size={14} className="text-slate-400 dark:text-gray-500" /> {selectedOrder.customer?.phone}</p>
                      <p className="flex items-start gap-2"><MapPin size={14} className="text-slate-400 dark:text-gray-500 mt-0.5" /> {selectedOrder.customer?.address}, {selectedOrder.customer?.district}</p>
                    </div>
                  ...
                  <div className="mb-2">
                    <h3 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider mb-3">Items to Pack ({selectedOrder.items?.length})</h3>
                    <div className="space-y-3">
                      {selectedOrder.items?.map((item:any, i:number) => (
                        <div key={i} className="flex gap-4 items-center p-3 border border-slate-200 dark:border-white/10 rounded-xl shadow-sm bg-white dark:bg-[#141d1a]">
                          <img 
                            src={getImageUrl(item.product?.imageUrl || item.product?.thumbnail)} 
                            className="w-18 h-18 object-cover rounded-lg border border-gray-200 dark:border-white/10 shadow-sm cursor-zoom-in hover:opacity-80 transition-opacity"
                            onClick={() => setExpandedImage(getImageUrl(item.product?.imageUrl || item.product?.thumbnail))}
                            alt="product"
                            title="Click to zoom"
                          />
                          <div className="flex-1">
                            <p className="text-sm font-bold text-slate-800 dark:text-gray-200">{item.product?.name}</p>
                            
                            <div className="mt-2 inline-block bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 px-3 py-1.5 rounded-md text-xs font-bold text-emerald-700 dark:text-emerald-400 shadow-sm">
                              Pack Quantity: 
                              <span className="mx-1.5 px-2.5 py-0.5 bg-emerald-600 dark:bg-emerald-500 text-white text-sm font-black rounded shadow-sm">
                                {item.quantity}
                              </span>
                              Piece
                            </div>
                ...
                <div className="p-4 border-t border-gray-200 dark:border-white/10 bg-slate-50 dark:bg-[#141d1a] flex gap-3 shrink-0">
                  <button 
                    ...
                    className="flex-1 py-3.5 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-amber-700 dark:text-amber-400 rounded-xl font-bold text-sm hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-colors flex items-center justify-center gap-2 shadow-sm"
                  >
                  ...
                  <button 
                    ...
                    className="flex-1 py-3.5 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 transition-colors flex items-center justify-center gap-2 shadow-sm"
                  >
                    <CheckCircle2 size={18} /> Pack Complete
                  </button>
                </div>
              </div>
            )}
      ...
      {activeTab === 'history' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-in fade-in duration-300">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-[#1a2421] p-5 rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col h-[700px] transition-colors">
            <div className="mb-4 pb-4 border-b border-gray-200 dark:border-white/10 flex flex-col md:flex-row md:items-start justify-between gap-3">
              <div>
                <h2 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                  <CheckCircle2 size={20} className="text-green-600 dark:text-emerald-400" /> Packed History
                </h2>
                <div className="flex gap-4 mt-2">
                  <p className="text-[11px] bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-gray-300 px-2 py-1 rounded font-bold uppercase tracking-wider">
                    Total Parcels: <span className="text-slate-900 dark:text-white">{displayedPackedOrders.length}</span>
                  </p>
                  <p className="text-[11px] bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-400 px-2 py-1 rounded font-bold uppercase tracking-wider flex items-center gap-1">
                    <Box size={12} /> Total Items: <span className="text-indigo-900 dark:text-indigo-300">{totalSareesPacked}</span>
                  </p>
                </div>
              </div>
            ...
            <form onSubmit={(e) => e.preventDefault()} className="relative mt-2 mb-3">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <div className="overflow-y-auto pr-2 space-y-3 flex-1 custom-scrollbar">
              {displayedPackedOrders.length === 0 ? (
                <div className="text-center py-20 text-slate-400 dark:text-gray-500 flex flex-col items-center gap-2">
                  <Package size={40} className="opacity-50" />
                  <p>এই ফিল্টারে কোনো প্যাক করা পার্সেল নেই!</p>
                </div>
              ) : (
                displayedPackedOrders.map((order) => {
                  ...
                  return (
                    <div
                      key={order.id}
                      onClick={() => setSelectedPackedOrder(order)}
                    ...
                    >
                      ...
                      <div className="flex-1 flex justify-between items-center gap-3 overflow-hidden">
                        ...
                        <div className="flex flex-col items-end shrink-0">
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isActive ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400' : 'bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-gray-400'}`}>
                            {date}
                          </span>
                          <span className="text-[9px] text-slate-400 dark:text-gray-500 font-medium mt-1">
                            {time}
            ...
            {!selectedPackedOrder ? (
              ...
              <div className="bg-white dark:bg-[#1a2421] rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col h-[700px] overflow-hidden relative transition-colors">
                ...
                <div className="p-6 flex-1 overflow-y-auto custom-scrollbar z-10">
                  <div className="mb-6">
                    <h3 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider mb-3">Customer Details</h3>
                    <div className="space-y-2 text-sm text-slate-700 dark:text-gray-300 bg-slate-50 dark:bg-[#141d1a] p-4 rounded-xl border border-slate-100 dark:border-white/5 transition-colors">
                      <p className="flex items-center gap-2"><User size={14} className="text-slate-400 dark:text-gray-500" /> {selectedPackedOrder.customer?.name}</p>
                      <p className="flex items-center gap-2"><Phone size={14} className="text-slate-400 dark:text-gray-500" /> {selectedPackedOrder.customer?.phone}</p>
                      <p className="flex items-start gap-2"><MapPin size={14} className="text-slate-400 dark:text-gray-500 mt-0.5" /> {selectedPackedOrder.customer?.address}, {selectedPackedOrder.customer?.district}</p>
                    </div>
                  </div>

                  <div className="mb-2">
                    <h3 className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-wider mb-3">Packed Items ({selectedPackedOrder.items?.length})</h3>
                    <div className="space-y-3">
                      {selectedPackedOrder.items?.map((item: any, idx: number) => (
                        <div key={idx} className="border border-slate-200 dark:border-white/10 rounded-xl p-3 flex gap-4 bg-slate-50 dark:bg-[#141d1a] shadow-sm items-center transition-colors">
                          <img 
                            src={getImageUrl(item.product?.imageUrl || item.product?.thumbnail)} 
                            className="w-16 h-16 object-cover rounded-lg border border-gray-200 dark:border-white/10 shadow-sm cursor-zoom-in hover:opacity-80 transition-opacity" 
                            onClick={() => setExpandedImage(getImageUrl(item.product?.imageUrl || item.product?.thumbnail))}
                            alt="product"
                            title="Click to zoom"
                          />
                ...
                <div className="p-4 border-t border-gray-200 dark:border-white/10 bg-slate-50 dark:bg-[#141d1a] flex flex-col gap-3 shrink-0 z-10 transition-colors">
                  {selectedPackedOrder.status?.toUpperCase() === 'PACKED' ? (
                    <div className="flex gap-3">
                      <button onClick={handleUnpack} className="flex-1 py-3.5 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-600 dark:text-rose-400 rounded-xl font-bold text-sm hover:bg-rose-600 hover:text-white transition-colors flex items-center justify-center gap-2 shadow-sm">
                        <RotateCcw size={18} /> Unpack (রিভার্ট)
                      </button>
                    </div>
                  ) : (
                    <div className="bg-slate-200/70 dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-xl p-3 text-center">
                      <p className="text-xs font-bold text-slate-600 dark:text-gray-400 flex items-center justify-center gap-1.5">
                        <Truck size={14} />
                        এই পার্সেলটি বর্তমানে '{selectedPackedOrder.status}' স্ট্যাটাসে আছে, তাই এটি আনপ্যাক করা সম্ভব নয়।
                      </p>
                    </div>
                  )}
                </div>
