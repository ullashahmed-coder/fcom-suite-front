...
export default function AddEditProductPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setUploadedImageUrl(data.imageUrl); // ব্যাকএন্ড থেকে আসা ইউআরএল সেভ করে রাখা
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(isEditMode ? "✅ প্রোডাক্ট আপডেট সফল হয়েছে!" : "✅ নতুন প্রোডাক্ট সফলভাবে যুক্ত হয়েছে!");
